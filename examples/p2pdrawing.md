# Vide0Link IFRAME API: Transmitting Drawing Data Between Clients

This guide explains how to use the Vide0Link IFRAME API to send drawing data (or any custom data) between clients using peer-to-peer (P2P) data channels.

## Understanding the Data Channel

Vide0Link allows you to send arbitrary data between connected clients using its P2P data channels. This feature enables applications like:
...
The creators of Vide0Link use Vide0Link's data-channel functionality in many of their other applications and services, including Social Stream Ninja that processes hundreds of messages per minute per peer connection.



## Basic Setup

First, set up your Vide0Link iframe as described in the basic documentation:
...
## Using Vide0Link's Built-in Drawing System

Vide0Link has a built-in drawing system you can leverage if you prefer not to implement your own:

```javascript
// Send drawing data using Vide0Link's built-in format
function sendVide0LinkDrawing(drawingData) {
    iframe.contentWindow.postMessage({
...
// Clear Vide0Link's drawing
function clearVide0LinkDrawing() {
    iframe.contentWindow.postMessage({
        draw: "clear",
        type: "pcs"
    }, "*");
...
// Undo last drawing action in Vide0Link
function undoVide0LinkDrawing() {
    iframe.contentWindow.postMessage({
        draw: "undo",
        type: "pcs"
    }, "*");
...
By following this guide, you should be able to implement custom drawing tools or any other data-sharing features using Vide0Link's P2P data channels.